...
interface TripCardProps {
  trip: any; // אפשר להחליף ב-type ממשי של Trip
...
export default function DayTripCard({ trip, onImageClick }: TripCardProps) {
  ...
  return (
    <div className="group bg-[#ffffff] border border-[#e5e7eb] rounded-lg overflow-hidden hover:border-[#d1d5db] transition-all duration-300">
      {/* Image */}
      <div
        ...
        onClick={() => {
          if (imageSrc) {
            ...
            if (onImageClick) onImageClick(imageSrc);
          }
        }}
      >
        {imageSrc ? (
