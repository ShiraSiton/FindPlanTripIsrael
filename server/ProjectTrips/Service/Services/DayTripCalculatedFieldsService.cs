...
namespace Service.Services
{
    public class DayTripCalculatedFieldsService : IDayTripCalculatedFieldsService
    {
        ...
        private async Task<int?> CalculateCommonRegionAsync(List<Place> places, List<Route> routes)
        {
            ...
            // מקבל רשימה של כל ה-RegionId של המקומות
            var regionIds = places.Select(p => p.RegionId).Distinct().ToList();
            ...
            // אחרת, צריך לעלות למעלה בהיררכיה
            while (true)
            {
                ...
                {
                    var region = await repositoryRegion.GetByIdAsync(id);
                ...
                // אם אין הורים, מחזירים null (אין אזור משותף)
                if (!parentIds.Any())
                    return null;

                // אם נשאר רק אזור אחד, זה האזור המשותף
                if (parentIds.Count == 1)
                    return parentIds.First();

                // עדכון הרשימה לאזורים החדשים להמשך הלולאה
                regionIds = parentIds;
            }
        }
