...
namespace Service.Services
{
    public class DayTripService : IDayTripService
    {
        ...
        public async Task<List<DayTripDto>> GetAllAsync()
        {
        ...
        public async Task<DayTripDto> GetByIdAsync(int id)
        {
            var trip = await repository.GetByIdAsync(id);
            if (trip == null)
                throw new KeyNotFoundException($"Day trip with id {id} not found");

            return mapper.Map<DayTripDto>(trip);
        }

        public List<DayTripDto> GetFilteredTrips(TripFilterDto filter)
        {
            // הבסיס: רק טיולים מאושרים
            var query = repository.GetFilteredDayTrips();

            // סינון לפי שדות אם קיימים
            ...
            if (filter.Duration != null && filter.Duration != 0)
                query = query.Where(d => d.TotalDurationHours <= filter.Duration); // תתאים לפי Duration שלך

            if (filter.LengthKM != null && filter.LengthKM != 0)
                query = query.Where(d => d.TotalLengthKM <= filter.LengthKM); // תתאים לפי Duration שלך

            if (filter.Price != null && filter.Price != 0)
                query = query.Where(d => d.Price <= filter.Price); // תתאים לפי Duration שלך

            if (filter.StopsCount != null && filter.StopsCount != 0)
                query = query.Where(d => d.DayTripItems.Count() <= filter.StopsCount); // תתאים לפי Duration שלך

            //if (filter.FamilyFriendly.HasValue)
            //    query = query.Where(d => d.FamilyFriendly == filter.FamilyFriendly.Value);
            ...
            return mapper.Map<List<DayTripDto>>(query.ToList());
        }
        ...
        public async Task<DayTripDto> UpdateRatingAsync(int id, UpdateRatingDto dto)
        {
            if (dto == null)
                throw new ArgumentNullException(nameof(dto));
            ...
            return mapper.Map<DayTripDto>(dayTrip);
        }

        public async Task<DayTripDto> SetApprovalStatusAsync(int userId, int id, ApprovalStatus newStatus)
        {
            var user = await repositoryUser.GetByIdAsync(userId);
            if (user == null)
        ...
        public async Task<DayTripDto> ApproveDayTripWithItemsAsync(int adminId, int dayTripId, string? rejectReason = null)
        {
            ...
            else
            {
                ...
                dayTrip.RejectReason = rejectReason ?? explanation;
                dayTrip.ApprovedAt = null;
            }
        ...
        private async Task<CalculateComputedFieldsResult> RecalculateAndReturnAsync(int dayTripId)
        {
            ...
            {
                DayTrip = mapper.Map<DayTripDto>(dayTrip),
